...
BEGIN_MESSAGE_MAP(CEdgeBrowserAppDlg, CWnd)
    ON_WM_PAINT()
    ON_WM_QUERYDRAGICON()
    ON_WM_SIZE()
    ON_WM_CREATE()
...
HRESULT CEdgeBrowserAppDlg::OnCreateCoreWebView2ControllerCompleted(HRESULT result, ICoreWebView2Controller* controller)
{
    if (result == S_OK)
    {
        ...
        HRESULT hresult = m_webView->Navigate(CEdgeBrowserAppDlg::_url);
...
int CEdgeBrowserAppDlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
